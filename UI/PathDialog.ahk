...
class PathDialog extends UI.BaseDialog
{
    ...
    define()
    {
        if (!this.data.Has("pathType")) {
            throw Error("ProgrammerException", A_ThisFunc, "data.pathType is missing, must be one of ['file', 'folder', 'directory']")
        }
        if (!InStr("file, folder, directory", this.data["pathType"])) {
            throw Error("ProgrammerException", A_ThisFunc, "data.pathType must be one of ['file', 'folder', 'directory']")
        }
